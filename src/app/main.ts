...
import App from './StagingApp.svelte';

const target = document.getElementById('app')!; // the mount point exists in every app/*.html shell

// CH16 cutover: the redesigned sidebar-shell app (StagingApp — renamed to App at the end of the
// cutover) is THE app on ALL surfaces. It is mode-aware internally (PAGE_MODE → real Store / seeded
// DemoStore / isolated staging DB), so a single mount covers app/demo/staging. The pre-cutover vanilla
// App.svelte + src/app/components/* are retired in the same PR.
mount(App, { target });
