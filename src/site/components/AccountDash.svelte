<script lang="ts">
  ...
  // ARCHIVE FREEZE (docs/archive-freeze.md): the shared freeze flag. While archived, this whole page
  // renders ONLY an archived notice in place of the login/signup/dashboard body below (kept intact,
  // never deleted, inside the `{#if !ARCHIVED}` branch) — see src/lib/archive.ts for the full
  // explanation + revert instructions.
...
<SiteShell active="account">
  ...
  {#if ARCHIVED}
    <!-- ARCHIVE FREEZE (docs/archive-freeze.md): replace the ENTIRE login/signup/dashboard body with
         a single archived notice — new accounts/subscriptions/sync are paused. The rest of this
         component (below, in the {:else} branch) is left completely intact for the revert. -->
    <div class="note warn" data-testid="archived-note">
      <p class="m-0 text-[15px] font-semibold text-foreground">Blotterbook is archived</p>
      <p class="mb-0 mt-2">{ARCHIVE_NOTE}</p>
      <p class="mb-0 mt-2">
        Already have an account with cloud sync? Your data is still yours — <a href="/app/app.html">open the app</a> to export it locally,
        and <a href="/help/support.html">contact support</a> to delete your account.
      </p>
    </div>
  {:else}
