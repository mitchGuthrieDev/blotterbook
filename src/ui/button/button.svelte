<script lang="ts" module>
  ...
  // tokens (via the @theme utilities — bg-primary/text-bg/border-line/…): `default` is the secondary
  ...
  export const buttonVariants = tv({
    base: 'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md border font-sans leading-none no-underline cursor-pointer select-none transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:opacity-50 disabled:cursor-not-allowed',
    variants: {
      variant: {
        default: 'border-line bg-panel2 text-txt hover:border-hover-line',
        primary: 'border-primary bg-primary text-bg font-semibold hover:opacity-90',
